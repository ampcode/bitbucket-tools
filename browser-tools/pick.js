...
switch (action) {
  case 'describe':
    console.log(JSON.stringify({
      name: 'browser_pick',
      description: 'Interactive element picker. Displays a message and allows the user to click on elements in the browser. Returns information about the selected element(s) including tag, id, class, text content, HTML, and parent hierarchy. Supports multi-select with Cmd/Ctrl+click.',
      inputSchema: {
        type: 'object',
        properties: {
          message: {
            type: 'string',
            description: 'Message to display to the user explaining what to select (e.g., "Click the submit button")'
          }
        },
        required: ['message']
      }
  ...
  case 'execute':
    ...
    process.stdin.on('end', async () => {
      try {
        ...
        if (!message) {
          console.error('Error: message parameter required');
          process.exit(1);
        }
